...
const FormBlog = () => {
  ...
  return (
    <Container>
      <h2 className="text-2xl mb-10 text-center">Create New Blog</h2>
      <form onSubmit={handleSubmit} className="max-w-lg  mx-auto">
        <div className="mb-4 space-y-4">
          {inputs.map((item) => (
            <Input value={values[item.name]} key={item.id} {...item} onChange={onChange} />
          ))}
        ...
        <button
          type="submit"
          className="inline-block mt-3 px-7 py-3 bg-blue-600 text-white font-medium text-sm leading-snug  rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out"
        >
          Publish
        </button>
      </form>
